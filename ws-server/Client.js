 ...
 *    [options.autoReconnect] 是否自动连接
 *    [options.autoReconnectInterval] 自动连接间隔时间 
 *    [options.channels] {Array} 选填。需要订阅的消息
 */
function Client(options) {
    ...
    //var socketURL,site;
    this.socketURL = this.options.url || 'ws://localhost:8080/ws';
}
 ...
 *   data.event 可能的值，addChannel(注册请求数据)/removeChannel(注销请求数据)/push(提交数据) 
 *   data.channel 可能的值, order(表示下单交易)/market(市场价格)/position（仓位）/wallet(账户信息)
 *   data.parameters  一些参数，比如{ appKey: "a",appSecret: "b", amount: 123,price: 234}
