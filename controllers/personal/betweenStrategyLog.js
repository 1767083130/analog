...
function list(req,res,callback){
    ...
    let query = { "$where": function(){
        ...
        if(sStatus){
            if(this.status != sStatus){
                return false;
            }
        }

        if(sAuto){ 
            for(var index in this.operates){
                var operate = this.operates[index];
                if(operate.orgOperate.id == this.currentStep){
                    if(sAuto == "0"){
                        if(operate.orgOperate.auto == true){
                            return false;
                        }
                    } else {
                        if(operate.orgOperate.auto == false){
                            return false;
                        }
                    }
