...
module.exports = function (router) {
    ...
    router.get('/depth.do',  async(function* (req, res) {
        ...
        try{
            ...
            let realPrice = yield* realTimePrice.getRealPrice(site,symbol);
            if(realPrice){
     ...
     * e.g variable: chbtc.btc#cny.buys[0]
     */
    router.get('/getVariableValue.do',  async(function* (req, res) {
        try{
            ...
            let val = yield* expression.calculate(variable,envOptions);
            let getRes = {
                isSuccess: true,
                value: val
            };
    ...
    router.post('/operate.do',  async(function* (req, res) {  
        try{
            ...
            let runRes = yield* operate.runOperates(expression,envOptions,true);
            res.json(runRes);
        } catch(err){
            console.error(err);
