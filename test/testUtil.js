...
let testUtil = new class{
        ...
        if(!transferStrategy){
            transferStrategy = new TransferStrategy({
                ...
                operates: [{
                    ...
                    previousOperate: 0,
                    nextOperate: 2,
                    orderAmount: 0.1 //"btctrade.eth.account.available"  //比如 5 或者 5%,数字表示多少个，百分比表示账户总额中的百分之多少
                },{
    ...
    * getTestAccount(reset){
     ...
     */
    * getTestStrategyLog(options){
        options = options || {};
        if(!options.strategy){
            options.strategy = yield* this.getTestTransferStrategy();
        }
        ...
        if(!options.accounts){
            options.accounts = yield* this.getTestAccount();
        }
        ...
        let conditionResult = yield* normalStrategyRunner.getConditionResult(condition,envOptions);
        let getLogRes = yield* baseStrategyRunner.getTransferStrategyLog(conditionResult.orders,options.strategy);
     ...
     * @returns {Order} 交易委托
     */
    * getTestOrder(side,stepAmount,options){
        let identifier;
        if(!options.strategy){
            options.strategy = yield* this.getTestTransferStrategy();
        }

        let strategyLog = yield* this.getTestStrategyLog(options);
