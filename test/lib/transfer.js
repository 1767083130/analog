...
describe.skip('支付测试. path: transfer.js', function () {
    ...
    it.skip('runOrderOperate 执行需要委托交易的差价策略的step', function (done) {
        co(function *(){
            let operateLog = {
             ...
             * @public
             */
            let res = yield* transfer.runOrderOperate(operateLog,identifier,stepAmount);
    ...
    it.skip('cancelOrder 取消委托', function (done) {
        co(function *(){
            /**
             * 取消订单
             * 
             ...
             * @public
             */
            let res = yield* transfer.cancelOrder(order,identifier);
            assert.equal(res.isSuccess,true);
            done();
        }).catch(function(e){
            done(e);
    ...
    it.skip('syncRecentOrders 同步最近委托状态', function (done) {
        co(function *(){
            /**
             ...
             * @public
             */
            yield* transfer.syncRecentOrders(function(err,res){
                assert.equal(res.isSuccess,true);
                done();
            });
