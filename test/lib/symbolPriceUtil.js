    ...
    it.only('getMarketPrice 获取在某个网站中，卖出一定数量的交易品种，市场能给出的价格', function (done) {
        co(function *(){
            ...
            let realPrices = {
                ...
                sells: [[4006,1],[4007,1],[4008,1]] //卖出委托，按照价格升序排序
            };
            marketPrice = yield* symbolPriceUtil.getMarketPrice('chbtc','btc#cny',1,'buy',1,realPrices);
            assert.equal(marketPrice.price,4006);

            marketPrice = yield* symbolPriceUtil.getMarketPrice('chbtc','btc#cny',3,'buy',1,realPrices);
