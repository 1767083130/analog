...
class Multiple {
     ...
     * 运行策略
     */
    * runStrategy(strategy,stepCallBack){
        ...
        let buySites = [],  //e.g [{ site: "chbtc",symbol: "btc#cny",sells: [[7800,8.9],[7798.6,12.5]],total: 100, available: 120 }]
            sellSites = []; //e.g [{ site: "chbtc",symbol: "btc#cny",buys: [[7800,8.9],[7798.6,12.5]],total: 100, available: 120  }]
        for(var i = 0; i < siteItems.length; i++){
            ...
            for(var j = 0; j < siteItem.symbols.length; j++){
                ...
                //获取行情信息
                let realPrice = yield* realTimePrice.getRealPrice(siteItem.site,symbolItem.symbol);
                if(!realPrice){
                    continue;
                }
    ...
    * getConditionResult(condition,envOptions) { 
        ...
        var variableValues = yield* this._getVarRealPrices(stack,envOptions.realPrices); //表达式的变量
        var indexs = [], //行情价格中的索引
            amounts = [], //能符合条件的委托价格
    ...
    * getStackItemValue(stackItem,conditionRealPrices,envOptions){
        return yield* super.getStackItemValue(stackItem,conditionRealPrices,envOptions);
    }
    ...
    * _getVarRealPrices(stack,realPrices){
        ...
        for (var i = 0; i < stack.length; i++) {
            ...
            if(assignPriceInfo){
                ...
                var side = assignPriceInfo.side;
                if(!existsVariable){
                    variables.push({ 
                        stackItem: stackItem, 
                        values: (side == 'buy' ? realPrice.buys : realPrice.sells),
                        site: assignPriceInfo.site,
                        symbol: assignPriceInfo.symbol,
                        side: assignPriceInfo.side
                    });
                }
            }
        }
