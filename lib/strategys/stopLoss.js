...
let stopLoss = new class{
    ...
    * runStrategy(stepCallBack){
        
        let stopLosses = yield* StopLoss.find({
            isValid: true,
        ...
        for(let stopLoss of stopLosses){
            try{
                let stepRes = yield* this.runStopLoss(stopLoss);
    ...
    * runStopLoss(stopLoss){
        for(let step of stopLoss.steps){
            let accountInfo = yield* account.getSiteAccount(stopLoss.userName,step.site,false);
            if(!accountInfo){
                continue;
            }
            step.stopLoss = stopLoss;
            
            let identifier = yield ClientIdentifier.getUserClient(stopLoss.userName,step.site);
            let marketPrice = yield* this.getMarketPrice();
            let res = yield* this.getStopLossLog(step,marketPrice,identifier);

            if(res.needStopLoss){
                yield* this.runStopLossLog(res.stopLossLog,marketPrice,identifier);
            }
        }
    }

    * getStopLossLog(step,marketPrice,identifier){
        ...
        */
        ...
        if(!identifier){
            identifier = yield ClientIdentifier.getUserClient(stopLossLog.userName,stopLossLog.site);   
            if(!identifier){
                return { isSuccess: false, message: "系统错误！"};
            }
        }
        ...
        // if(!accountInfo){
        //     accountInfo = yield account.getSiteAccount(stopLossLog.userName,stopLossLog.site,false);
        //     if(!accountInfo){
        //         return { isSuccess: false, message: "系统错误！"};
        //     }
        // }
        if(!marketPrice){
            marketPrice = yield* this.getMarketPrice();
        }
    ...
    * runStopLossLog(stopLossLog,marketPrice,identifier){
        ...
        if(operateType == 'trade'){ //需要委托交易
            let account = yield account.getSiteAccount(stopLossLog.userName,stopLossLog.site,false);
            ...
            let order = { 
                ...
                price: consignPrice, 
                side: stopLossStep.side,
                reason: "stoploss", 
                symbol: stopLossLog.symbol
            };

            let res = yield* order.createOrder(order);
            if(res.isSuccess){
            ...
            newLogModel = yield newLogModel.save();
    ...
    * getMarketPrice(){
        let realPrice = yield* realTimePrice.getRealPrice(stopLossLog.site,stopLossLog.symbol);
        if(!realPrice){
            return { isSuccess: false, errorCode: "100004", message: "获取行情信息失败" };
        }

        marketPrice = realPrice.buys[0];
    }
