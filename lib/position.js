...
let position = new class{
     ...
     */
    getPositions(site,lazyMode = true){
        ...
        for(let sitePositions of positionsInfo){
            ...
            for(let position of sitePositions.items){
                ...
                if(unit.coin == symbolParts.settlementCoin){
                    ...
                    targetCoinAmount = new Decimal(targetCoinAmount).div(getSymbolPriceRes.price).toNumber();
     ...
     */
    * getSymbolHoldAmount(position,realPrice){
        ...
        if(symbolParts.targetCoin == unitName){
            ...
            if(!realPrice){
                realPrice = yield* realTimePrice.getRealPrice(position.site,position.symbol);
            }
            if(!realPrice){
            ...
            let isEnd = false;
            let steps = (position.positionType == 1 ? realPrice.sells : realPrice.buys);
    ...
    * syncPositions(userName,site){
     ...
     * @api public
     */
    * getAmountUsable(userName,site,options){
        let amountsLeft = []; //账户可使用的剩余金额
        let account = options.account;
        if(!account){
            account = yield account.getSiteAccount(userName,site);
        }

        let orgPositions = options.positions;
        if(!orgPositions){
            orgPositions = yield* this.getSitePositions(userName,site);
        }

        let platform = configUtil.getPlatform(site);
        ...
        for(let coinItem of accountCoins){
            ...
            let coinValue = yield* this.getCoinsValue(site,[{ coin: coinItem.coin,amount: coinAmount }],'sell',options);
        ...
        for(let position of positions){
            let coinValue = yield* this.getCoinsValue(site,[{ symbol: position.symbol,amount: position.amount }],'buy',options);
            if(Number.isNaN(coinValue)){
                return;
            }
        ...
        for(let symbolItem of platform.symbols){
            ...
            let settlementCoinValue = yield* this.getCoinsValue(site,[{ symbol: symbol,amount: assetTotal }],'sell',options);
            if(Number.isNaN(settlementCoinValue)){
                return;
            }

            //计算可以做多的数量 
            buyAvailable = yield* this.getCoinsValue(site,[{ symbol: symbolInfo.fullSymbol,amount: settlementCoinValue }],'buy',options);
            if(Number.isNaN(buyAvailable)){
                return;
            }

            //计算可以做空的数量 
            sellAvailable = yield* this.getCoinsValue(site,[{ symbol: symbolInfo.fullSymbol,amount: settlementCoinValue }],'sell',options);
     ...
     * @api private
     */
    * getCoinsValue(site,accountCoins,side,options){
        ...
        for(let coinItem of accountCoins){
            ...
            if(targetCoin != standardCoin){ //如果不是本位币，则根据市场价格换算成本位币
                ...
                itemTotal = yield* this.getSiteMarketValue(site,symbol,side,coinItem.amount,null,options.stepPercent || STEP_PERCENT,options.realPrice);
     ...
     */
    * getSiteMarketValue(site,symbol,side,targetCoinAmount,settlementCoinAmount,percent,realPrice){
        if(!targetCoinAmount && !settlementCoinAmount){
            throw new Error('参数错误,targetCoinAmount和settlementCoinAmount不能同时为空');
        }

        if(!realPrice){
            realPrice = yield* realTimePrice.getRealPrice(site,symbol);
        }
        if(!realPrice){
            return Number.NaN;
        }
        ...
        let isEnd = false;
        let steps = (side == 'buy' ? realPrice.sells : realPrice.buys);
