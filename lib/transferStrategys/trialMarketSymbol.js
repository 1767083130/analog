...
const Decimal = require('decimal.js'),
    ...
    appConfigUtil = require('../utils/configUtil');
...
let trialMarketSymbol = new class{
     ...
     * @returns {Object} e.g  
     *   {isSuccess: true, 
     *    symbolPathPairs: [{ symbol: "bch#btc",reverse: false},{ symbol: "bch#eos", reverse: true}] } 
     */
    getSymbolPathPairs(siteA,siteB,realPrices){
        //根据site分组
        let groups = [];
        realPrices.forEach(value => {
            let groupItem = groups.find(p => p.site == value.site);
            if(groupItem){
                groupItem.depths.push(value);
            } else {
                groups.push({
                    site: value.site,
                    depths: [value]
        ...
        let symbolPathPairs = this._getSymbolPathPairs(groups[0],groups[1]);
        return symbolPathPairs;
    }

    /**
     * 获取SymbolPathPairs
     * @param {*} siteGroupA 网站A的行情深度 e.g { site: "baidu",depths: [{symbol: "btc#usd",buys:[],sells:[]}]}
     * @param {*} siteGroupB 网站B的行情深度 e.g { site: "baidu",depths: [{symbol: "btc#usd",buys:[],sells:[]}]}
     * @param {*} depths 
     * @returns e.g {siteA: 'baidu', siteB: 'qq',symbolPathA: [{symbol: "bch#btc",reverse: false}],symbolPathB: [] }
     */
    _getSymbolPathPairs(siteGroupA,siteGroupB){
        let symbolPathPairs = [],
     ...
     * @param {Object} siteDepths，某一个网站的市场行情信息 e.g [{ site:"baidu",symbol: "btc#usd",buys[],sells:[]}]
     * @returns {Object} e.g  
     *   {isSuccess: true, 
     ...
     * 获取两个网站各自的兑换路径
     * 
     * @param {Object} siteGroupA e.g { site: "baidu",depths: [{symbol: "btc#usd",buys:[],sells:[]}]}
     * @param {Object} siteGroupB e.g { site: "qq",depths: [{symbol: "btc#usd",buys:[],sells:[]}]}
    ...
    getPathPairs(siteGroupA,siteGroupB){
        ...
        //去掉重复的路径
        let copyPathPairs = [],copyPathPairs1 = [];
