...
class BaseStrategyRunner {
     ...
     * @public
     */
    * runStrategy(strategy,envOptions){
        let res = { isSuccess: true },condition;
        //let transferStrategyLog = yield* this.getStrategyOrders(strategy);
        let env = { userName: strategy.userName };

        envOptions = envOptions || {};
        envOptions.env = envOptions.env || env;
 
        if(strategy.conditions){
            condition =  strategy.conditions.join(' & ');
        }
        let conditionResult = yield* this.getConditionResult(condition,envOptions);
        let conditionOrders = conditionResult.orders;
        if(conditionOrders.length == 0){
            return { isSuccess: true,message: "成功运行，策略没有满足交易条件"}
        }

        let getLogRes = yield* this.getTransferStrategyLog(conditionOrders,strategy);
    ...
    * runOperate(operateLog,identifier,stepAmount){
        ...
        if(operate.action == "trade"){
            res = yield* order.runOrderOperate(operateLog,identifier,stepAmount)
     ...
     * 
     * @returns 变量的详情，如 { site: "huobi",symbol: "btc#cny", variable: "buy",side:"buy" },表示huobi网站中btc#cny的价格,
    ...
    * getTransferStrategyLog(conditionOrders,strategy) { 
        ...
        for(var i = 0; i < strategy.operates.length; i++){
            ...
            var conditionOrder =  conditionOrders.find(function(item){
                let side = (operate.side == 'buy' ? 'sell' : 'buy'); 
                return item.site == operate.site && item.symbol == operate.symbol && item.side == side;
            }) 
            if(!conditionOrder){
            ...
            let amount = yield* this.getOperateAmount(operate.orderAmount,conditionOrders,envOptions);
            //let amount0 =   conditionOrder.amount * (operate.orderAmount || 100) / 100;
            ...
            if(operate.action == 'transfer'){ //操作类型。分为 trade(成交)、transfer（转币）两种
                ...
                if(amount <= 0 || amount < operate.minOrderAmount) {
                    //isContinue = false;
                    res.message = `账户余额不足`;
                    res.isSuccess = false;
                }
                ...
                let sellCoinLeft,buyCoinLeft,
                    coin = arr[0];
                if(operate.side == "sell"){ //卖出时不需要考虑资金是否充足,但不能超账户剩余数量
                ...
                if(operate.id == 1 || operate.validAccount){
                    if(amount <= 0 || amount < operate.minOrderAmount) {
                        res.isSuccess = false;
                        res.message = `网站${operate.site}账户余额不足。sellCoinLeft:${sellCoinLeft},可买数量为${amount},但每单的最小量为${operate.minOrderAmount}`;
                        //isContinue = false;
                    }
                }
                ...
                if( (isFirstOperate && (operate.minOrderAmount <= totalAmount))
                   || !isFirstOperate){
                //if(!isFirstOperate || operate.minOrderAmount <= totalAmount){
                    //成交金额
    ...
    * getOperateAmount(amountExpression,conditionOrders,envOptions){
        let stack = expression.getConditionStacks(amountExpression);
        let stackNew = [];

        let getVarValueMethod = function* (stackItem){
            return yield* this.getStackItemValue(stackItem,conditionOrders,envOptions);
        }
        ...
        for(let j = 0; j < stack.length; j++){
            ...
            let stackItemValue =  yield* this.getStackItemValue(stackItem,conditionOrders,envOptions); // 
     ...
     *     
     */
    * getStackItemValue(stackItem,conditionRealPrices,envOptions){ // [{ site: "huobi", symbol: 34,price: 324,amount: 34 }]
        var item = {}, //处理后的值
            itemValue,
            expressionType;
        ...
        if(expressionType == 'variable'){
            ...
            //如:{ site: "huobi",symbol: "btc#cny", variable: "buy",side:"buy" }
            ...
            if(assignPriceInfo){  //如果表达式表示的是符合条件的委托价格 如 huobi.btc.buy或huobi.btc.sell
                let realPrice = conditionRealPrices.find(function(value){
                    return value.symbol == assignPriceInfo.symbol 
                            ...
                            && value.side == assignPriceInfo.side;
                });
                ...
                let variableValue = yield* expression.getVariableValue(stackItem,envOptions);
     ...
     * 
     * @returns 变量的详情，如 
         { site: "huobi",symbol: "btc#cny", variable: "buy",side:"buy" },表示huobi网站中btc#cny的价格,
     *    variable暂时支持 buy（买入价）、sell（卖出价）
     */
