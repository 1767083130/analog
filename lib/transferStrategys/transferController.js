...
let transferController = new class {
    ...
    * runStrategy(strategy){
     ...
     * @public
     */
    * getConditionResult(condition,strategyType,envOptions){
        let instance = BaseStrategyRunner.getInstance(strategyType);
        return yield* instance.getConditionResult(condition,envOptions);
    }
    ...
    * onOrderStatusChanged(e){
        if(e.order.reason != 'transfer'){
            return;
        }

        let identifier = e.identifier;
        if(!identifier){
            identifier = yield clientIdentifier.getUserClient(e.order.userName,e.order.site);
        }
        ...
        //let operateId = order.operateId;
        let transferStrategyLog = yield TransferStrategyLog.findOne({_id: e.order.actionId });
        ...
        let currentLogOperate = transferStrategyLog.operates.find(function(value){
            return value._id.toString() == e.order.operateId.toString();
        });
    ...
    * finishTransferStep(operateLog){
     ...
     */
    * onOrderDelayed(e){
        if(['consign','wait','part_success'].indexOf(e.order.status) != -1){
            return;
        }

        let transferConfig = configUtil.getBusiness().transfer;
        if(e.order.reason != 'transfer' 
          || e.timeDelayed > transferConfig.maxOrderWait
          || e.timeDelayed < transferConfig.minOrderWait){ //被延迟10分钟
            return;
        }

        let strategyLog = yield TransferStrategyLog.loadByOperateId(e.order.actionId);
        if(strategyLog){
            //todo 这里系统异常处理
            console.log(`未找到_Id为${e.order.actionId}的log`);
            return;
        }
        ...
        if(!operate){
            console.log(`未找到_Id为${e.order.actionId}的operate`);
            return;
        }
        ...
        if(operate.id == 0 || operate.id == 1){
            //尚未执行任何操作，则撤销
            yield* order.cancelOrder(e.order,identifier);
        
        } else {
            if(order.autoRetry){
                //todo 这里是有问题的.不成功则重新发起委托,有可能会导致委托价格不可控
                //已经不能撤销了，则重新执行委托
                yield* order.retryOrder(e.order,identifier,0.2); 
            } else {
                ...
                let exchangeException = new ExchangeException({
                    ...
                    site: order.site,
                    actionId: order._id, //orderId or transferId
                    operateId: order.operateId, //transferStrategyLog.operate._id
