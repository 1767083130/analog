...
let operate = new class {
     ...
     */
    * runOperates(expression,envOptions,isTransaction){
        ...
        for(let operate of operates){
            Object.assign(operate.args,operate.args,envOptions);
            switch(operate.funName.toLowerCase()){
            case 'buy':
                res = yield* this.buy(operate.args);
                break;
            case 'sell':
                res = yield* this.sell(operate.args);
                break;
            case 'withdraw':
                res = yield* this.withdraw(operate.args);
                break;
            default:
    ...
    * buy(options){
        options.side = 'buy';
        return yield* this._createOrder(options);
    }
    ...
    * _createOrder(options){
        let identifier = yield clientIdentifier.getUserClient(options.userName,options.site);  
        if(!identifier){
         ...
         * @returns {Object} 是否成功。如{
             ...
             order: newOrder } //委托交易
         */
        let res = yield* order.createOrder(orderItem,identifier);
        // if(res.isSuccess){
