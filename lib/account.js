...
let account = new class{
     ...
     * @api public
     */
    * refreshAccountTrading(order, type){
    ...
    * refreshAccountTransfering(transfer, type){
        ...
        sourceAccount = yield Account.getUserAccount(transfer.userName,transfer.source);
        if(transfer.target == transfer.source){
            targetAccount = sourceAccount;
        } else {
            targetAccount = yield Account.getUserAccount(transfer.userName,transfer.target);
        }
        ...
        if(hasChanged){
            sourceAccount = yield sourceAccount.save();
            targetAccount = yield targetAccount.save();
        }
    ...
    * syncAllAccounts(stepCallback){
        ...
        for(let clientIdentifier of clientIdentifiers){
            let methodType = configUtil.getMethodType(siteItem.site,'market',1);
    ...
    * getMarketAccountInfo(userName) {  //todo
        ...
        //计算账户证券资产总值
        let accounts = yield Account.find({ userName: userName, isTest: false});
