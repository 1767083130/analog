...
let transferStatusChecker = new class {
    ...
    * checkStatus(transfer,options){
        ...
        */
        ...
        if(!identifier){
            identifier = yield clientIdentifier.getUserClient(transfer.userName,transfer.target);   
            if(!identifier){
        ...
        //同步委托状态
        yield* order.syncUserRecentOrders(transfer.userName,[transfer.target]);
        ...
        let orderChanges = yield AccountHistory.aggregate(
            { 
                $match: { 
        ...
        let amounts = [];
        if(!transfers){
            transfers = yield Transfer.find({userName: transfer.userName, status:{ $in:['consign','part_success'] } });
        }
